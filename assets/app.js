...
register('custom-content', {
  ...
  _handleAddItems: function (e, bundle) {
    e.preventDefault();
    console.log("_handleAddItems", bundle);
  ...
  constructor() {
    ...
    _defineProperty(this, "renderNewCart", async cartHTML => {
      ...
      .then(cart => {
        ...
        if(document.querySelectorAll(".scd__giftrow .gift-item").length > 0) {
          document.querySelectorAll(".scd__giftrow .gift-item").forEach(function(obj) {
            gift_cnt ++;
            var p = obj.getAttribute("data-price");
            if(cart.total_price > p) {
          ...
          document.querySelectorAll(".scd__giftrow .gift-item").forEach(function(obj) {
            var p = parseInt(obj.getAttribute("data-price"));
            if(cart.total_price < p && ff == 0) {
              document.querySelector(".away_price").textContent = formatMoney((p - cart.total_price), cart_MinimogSettings.money_format);
              document.querySelector(".away_title").textContent = obj.getAttribute("data-title");
              ff = 1;
            }
          });
        ...
        if(document.querySelector(".scd__freeshipping")) {
          var cart_price = parseInt(cart.total_price);
          var freeshipping_price = parseInt(document.querySelector(".scd__freeshipping").getAttribute("data-freeshipping_price"));
          var away_price = freeshipping_price - cart_price;
          if(away_price > 0) {
            var progress_percent = cart_price * 100 / freeshipping_price;
            document.querySelector(".away_price").textContent = formatMoney(away_price, cart_MinimogSettings.money_format);
            document.querySelector(".scd__freeshipping").classList.remove("complete");
            document.querySelector(".scd__freeshipping .progress_amount").style.width = progress_percent + "%";
          }
          else {
            document.querySelector(".scd__freeshipping").classList.add("complete");
          }
        }

        if(cart.item_count <= 0) {
          document.querySelector(".scd__cart").classList.add("empty");
        }
        else {
          document.querySelector(".scd__cart").classList.remove("empty");
        }
      })
      .catch((error) => {
...
ProductForm.prototype._setIdInputValue = function (value) {
  console.log("_setIdInputValue", value);
...
class Product {
  constructor(productForm) {
    ...
    _defineProperty(this, "handleSelectVariant", e => {
      ...
      document.dispatchEvent(new CustomEvent('variantChanged', { detail: newVariant }));
